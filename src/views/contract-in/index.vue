...
<template>
    <div class="permission-index"
         v-if="contractInData && contractInData.data">
        <Row>
            <Col span="24">
            <Card>
                <div slot="title"
                    ...
                    <Button type="primary"
                            size="default"
                            @click="handleAdd()">物资入库</Button>
                </div>
                <Table :columns="columns"
                       size="large"
                       no-data-text="暂无数据"
                       no-filtered-data-text="没有符合条件的数据"
                       :data="contractInData.data"></Table>
                <Page :total="contractInData.count"
                    ...
                    <span slot="">共
                        <span style="font-size: 20px; color: red; padding: 0 4px;">{{contractInData.count}}</span>条数据</span>
                </Page>
            </Card>
            </Col>
        </Row>
